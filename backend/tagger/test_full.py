#!/usr/bin/env python3
"""
Full test for Tagger agent via Lambda — OpenAI model.
"""
        ...
        for inst in test_instruments:
            ...
            if instrument:
                if instrument.get('allocation_asset_class'):
                    print(f"  [OK] {inst['symbol']}: Tagged successfully")
                    print(f"     Asset: {instrument.get('allocation_asset_class')}")
                    print(f"     Regions: {instrument.get('allocation_regions')}")
                else:
                    print(f"  [FAIL] {inst['symbol']}: No allocations found")
            else:
                print(f"  [WARN] {inst['symbol']}: Not found in database")
