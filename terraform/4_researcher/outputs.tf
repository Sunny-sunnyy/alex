...
output "scheduler_status" {
  ...
  value = !local.researcher_deployed ? "Disabled - deploy the researcher image first" : (
    var.scheduler_enabled ? "Enabled - Running every 12 hours" : "Disabled"
  )
}
...
output "setup_instructions" {
  ...
  value = local.researcher_deployed ? format(
    ...
    var.scheduler_enabled ? "⏰ Automated research is running every 12 hours" : "💡 To enable automated research, set scheduler_enabled = true"
