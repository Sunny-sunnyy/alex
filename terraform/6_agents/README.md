...
**Đã migrate từ Bedrock sang OpenAI.** Mỗi Lambda được inject `MODEL_ID_<AGENT>` riêng, không còn dùng `BEDROCK_MODEL_ID` hay `BEDROCK_REGION`. Bedrock IAM policy đã bị xoá. `OPENAI_API_KEY` là bắt buộc cho tất cả agent.
...
flowchart TD
    ...
    SQS -.redrive on failure.-> DLQ[aws_sqs_queue.analysis_jobs_dlq]
    ...
    OA[OpenAI API] -.model calls.-> PLAN
    OA -.model calls.-> TAG
    OA -.model calls.-> REP
    OA -.model calls.-> CHA
    OA -.model calls.-> RET
...
| `aws_iam_role_policy.lambda_agents_policy` | Inline IAM policy | Gói quyền CloudWatch Logs, SQS receive/delete, Lambda invoke, Aurora Data API, Secrets Manager, S3/S3 Vectors, SageMaker invoke. **Không còn Bedrock policy.** |
...
| `aws_lambda_function.planner` | Lambda | Orchestrator chính, timeout 900s, memory 2048MB, trigger từ SQS. Inject `MODEL_ID_PLANNER`. |
...
| `aws_lambda_function.tagger` | Lambda | Specialist phân loại instrument, timeout 300s, memory 1024MB. Inject `MODEL_ID_TAGGER`. |
| `aws_lambda_function.reporter` | Lambda | Specialist viết report markdown, timeout 300s, memory 1024MB. Inject `MODEL_ID_REPORTER`. |
| `aws_lambda_function.charter` | Lambda | Specialist sinh chart payload, timeout 300s, memory 1024MB. Inject `MODEL_ID_CHARTER`. |
| `aws_lambda_function.retirement` | Lambda | Specialist phân tích retirement, timeout 300s, memory 1024MB. Inject `MODEL_ID_RETIREMENT`. |
...
| Lambda invoke | `arn:aws:lambda:${var.aws_region}:${account_id}:function:alex-*` | Planner dùng để gọi specialist Lambdas. |
...
| S3 | `arn:aws:s3:::${var.vector_bucket}` và `/*` | `GetObject`, `ListBucket`. |
| S3 Vectors API | `arn:aws:s3vectors:${var.aws_region}:${account_id}:bucket/${var.vector_bucket}/index/*` | `QueryVectors`, `GetVectors`; reporter dùng tra cứu research chunks. |
| SageMaker | endpoint ARN từ `var.sagemaker_endpoint` | `InvokeEndpoint`; reporter dùng cho embeddings. |

**Đã xoá Bedrock IAM.** Agents không còn gọi AWS Bedrock — tất cả model calls đi qua OpenAI API. `OPENAI_API_KEY` được inject vào từng Lambda làm credential.
...
| --- | --- | --- | --- | --- | --- | --- |
| `MODEL_ID_PLANNER` | Yes | No | No | No | No | Model cho planner (default `openai/gpt-5.4-mini`). |
| `MODEL_ID_TAGGER` | No | Yes | No | No | No | Model cho tagger (default `openai/gpt-5.4-nano`). |
| `MODEL_ID_REPORTER` | No | No | Yes | No | No | Model cho reporter (default `openai/gpt-5.4-nano`). |
| `MODEL_ID_CHARTER` | No | No | No | Yes | No | Model cho charter (default `openai/gpt-5.4-nano`). |
| `MODEL_ID_RETIREMENT` | No | No | No | No | Yes | Model cho retirement (default `openai/gpt-5.4-nano`). |
| `OPENAI_API_KEY` | Yes | Yes | Yes | Yes | Yes | Credential cho OpenAI API — **bắt buộc**. |
...
| `VECTOR_BUCKET` | Yes | No | No | No | No | S3 Vectors bucket name từ Part 3. |
| `SAGEMAKER_ENDPOINT` | Yes | No | Yes | No | No | Planner giữ env này nhưng reporter mới là nơi dùng trực tiếp. |
...
| `model_config` | Map model được gán cho từng agent (planner, tagger, reporter, charter, retirement). |
| `setup_instructions` | Chuỗi hướng dẫn test/deploy sau `terraform apply`, bao gồm model configuration. |

## Các biến cần điền trong `terraform.tfvars`

`terraform.tfvars.example` hiện yêu cầu các biến sau:

| Biến | Bắt buộc | Default/Sample | Nguồn lấy giá trị |
| --- | --- | --- | --- |
...
| `model_id_planner` | No | `openai/gpt-5.4-mini` | Có default; override nếu cần model khác. |
| `model_id_tagger` | No | `openai/gpt-5.4-nano` | Có default. |
| `model_id_reporter` | No | `openai/gpt-5.4-nano` | Có default. |
| `model_id_charter` | No | `openai/gpt-5.4-nano` | Có default. |
| `model_id_retirement` | No | `openai/gpt-5.4-nano` | Có default. |
| `openai_api_key` | **Yes** | `sk-...` | OpenAI API key — bắt buộc cho model calls. |
| `sagemaker_endpoint` | No | `alex-embedding-endpoint` | Có default trong `variables.tf`. |
...
- `openai_api_key` đã được điền (không để trống)
- `polygon_api_key` không để placeholder nếu muốn planner cập nhật giá thật

## Model mapping hiện tại

| Lambda | Env var | Model khuyến nghị | Ghi chú |
| --- | --- | --- | --- |
| `alex-planner` | `MODEL_ID_PLANNER` | `openai/gpt-5.4-mini` | Orchestration — cần reasoning tốt nhất. |
| `alex-tagger` | `MODEL_ID_TAGGER` | `openai/gpt-5.4-nano` | Structured classification, scope hẹp. |
| `alex-reporter` | `MODEL_ID_REPORTER` | `openai/gpt-5.4-nano` | Tools + long-form report; nâng lên `mini` nếu chất lượng chưa đạt. |
| `alex-charter` | `MODEL_ID_CHARTER` | `openai/gpt-5.4-nano` | JSON chart payload cần nhanh và rẻ. |
| `alex-retirement` | `MODEL_ID_RETIREMENT` | `openai/gpt-5.4-nano` | Phân tích retirement; có thể dùng `4.1-nano` để tiết kiệm. |
...
| --- | --- |
| `backend/planner` | Terraform deploy `alex-planner`, gắn SQS trigger, inject `MODEL_ID_PLANNER` + DB/Polygon/observability env. |
| `backend/tagger` | Terraform deploy `alex-tagger`, inject `MODEL_ID_TAGGER` + DB/observability env. |
| `backend/reporter` | Terraform deploy `alex-reporter`, inject `MODEL_ID_REPORTER` + DB/SageMaker/observability env. |
| `backend/charter` | Terraform deploy `alex-charter`, inject `MODEL_ID_CHARTER` + DB/observability env. |
| `backend/retirement` | Terraform deploy `alex-retirement`, inject `MODEL_ID_RETIREMENT` + DB/observability env. |
...
Luồng tối thiểu để deploy folder này:
...
# Điền các giá trị thực tế vào terraform.tfvars
terraform init
terraform plan
terraform apply
terraform output
...
- `terraform output model_config` hiển thị đúng model từng agent

## Tóm tắt

`terraform/6_agents` là lớp hạ tầng tập trung cho Agent Orchestra của Part 6: SQS, IAM, S3 package bucket, 5 Lambda agents, SQS trigger cho planner, và log groups. Đã migrate hoàn toàn từ Bedrock sang OpenAI — mỗi Lambda nhận `MODEL_ID_<AGENT>` riêng, Bedrock IAM đã bị xoá, `OPENAI_API_KEY` là bắt buộc.
